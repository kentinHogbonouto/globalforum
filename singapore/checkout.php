...
class SendMail
{
  ...
  private function formatHTML($template, $domaine, $cart)
  {
    $symbole = $_POST['currencySymbole'];
    $currency = $_POST['currency'];

    $template = str_replace("{payment_id}", substr($this->payment_id, 0, 6), $template);
    ...
    foreach ($items as $value) {
      ...
      $item = str_replace("{price}", $symbole . $price, $item);
    ...
    $template = str_replace("{total}", $symbole . $subtotal + 160, $template);
  ...
  public function start($cart, $domaine)
  {
    try {
      ...
      $base_template = file_get_contents("base-email.html");
      $template = $this->formatHTML($template, $domaine, $cart);
      $base_template = $this->formatHTML($base_template, $domaine, $cart);
      $day = date('l, F jS Y');
      $hours = date('H:i:s');
      $recever_name = $cart->getOwner()['fist_name'] . ' ' . $cart->getOwner()['last_name'];
